...
# Calculate the number of days since the minimum date and convert to integer
data['Days'] = (data['datetime'] - data['datetime'].min()).dt.days


# Check for datetime columns and exclude them from features
x = data['Days']
y = data['flow']
...
future_x = pd.DataFrame({'x': np.arange(2148, 4148)})
future_predictions = model.predict(future_x)

fig, ax = plt.subplots()
ax.scatter(x2, y, label='Streamflow', marker='.')
ax.plot(x, predictions, color='red', label='Linear Regression Observed')
ax.plot(future_x, future_predictions, color='purple', label='Forecast')
ax.set_xlabel('Days since 1 Jan 2018')
...
mean_22 = calc_mean(data, time_period, 'flow', start_date, end_date)
max_22 = calc_max(data, time_period, 'flow', start_date, end_date)
min_22 = calc_min(data, time_period, 'flow', start_date, end_date)

start_date = '2023-11-01'
end_date = '2023-11-19'
time_period = 'W'

mean_23 = calc_mean(data, time_period, 'flow', start_date, end_date)
max_23 = calc_max(data, time_period, 'flow', start_date, end_date)
min_23 = calc_min(data, time_period, 'flow', start_date, end_date)
...
# %%
# Get Weekly Mean Flow for Oct and Nov

# Set variables
year = 2018
month = 10

# Resample original data by week and calc max, min, mean
wmax_flow = data['flow'].resample('W').max()
wmin_flow = data['flow'].resample('W').min()
wmean_flow = data['flow'].resample('W').mean()

# Filter for the month of Oct
...
# Calculate min, max, and mean for recent years in Oct
...
# Save cacluations to dataframes for easier use
...
# Filter for recent years in Nov
...
ax1.set(title="Oct Weekly Flow Since 2018", xlabel="Date",
...
ax2.set(title="Nov Weekly Flow Since 2018", xlabel="Date",
        ylabel="Flow [cfs]")
ax2.grid(True)
...
monthly_data = data['2023-10-01':'2023-11-11']
last_week = data['2023-11-05':'2023-11-11']
last_4weeks = data['2023-10-15':'2023-11-11']
...
week1_fcst = int(nov_means - (monthly_data.loc['2023-11-11', 'flow']) +
                 last_week_avg*0.75)
week2_fcst = int(nov_means - (monthly_data.loc['2023-11-11', 'flow']) +
                 last_4weeks_avg)
