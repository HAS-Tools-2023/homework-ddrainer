...
# %%
# Today we will explore data access using APIs. 
...
data = pd.read_table(filepath, sep='\t', skiprows=30, names=['agency_cd', 'site_no','datetime', 'flow', 'code'],parse_dates=['datetime'], index_col='datetime')

# %%
# Option two:
#  We can actually just read directly from the URL that this is coming from
# NOTE: this only works when its formatted as something readable. (To test this: See what happens if you click table rather than tab separated.) 
...
data3 = pd.read_table(url2, skiprows=30, names=['agency_cd', 'site_no', 'datetime', 'flow', 'code'],parse_dates=['datetime'], index_col=['datetime'])

# What we are doing here is actually just interacting with the USGS's Rest API:
