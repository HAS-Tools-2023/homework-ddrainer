...
criteria_1 = (flow_data[:, 3]>0) & (flow_data[:, 2] <= 15) & (flow_data[:,1] == 9)
pick_data = flow_data[criteria_1, 3]
flow_mean_first = np.nanmean(pick_data)
flow_median_first = np.nanmedian(pick_data)

print(flow_mean_first), print(flow_median_first)

criteria_2 = (flow_data[:, 3]>0) & (flow_data[:, 2] > 16) & (flow_data[:,1] == 9)
pick_data = flow_data[criteria_2, 3]
flow_mean_second = np.nanmean(pick_data)
flow_median_second= np.nanmedian(pick_data)

print(flow_mean_second), print(flow_median_second)
